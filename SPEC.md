...
## Stack
- Application ID / package: `com.tekphreak.darkbook`
...
> including a few features added after the initial build (image
> attachments, location capture, font selection, a generic long-press
> share, and a splash-screen preview toggle in Settings).
...
2. **Entry List** — reverse-chronological list of entries, black background (#000000), white text (#FFFFFF), minimal dividers; each row shows the date, the captured coordinates (if any), and a body preview
...
5. **Settings** — reachable via the entry list's overflow menu: long-press-to-share toggle, font picker, and a "Show Splash Screen" preview button
...
3. **Build the release artifact** — produce a signed `.aab` (Android App Bundle), not a `.apk`: `./gradlew bundleRelease`.
4. **Store listing** — app name, short/full description, screenshots (phone + optional tablet), feature graphic, icon, category (likely "Lifestyle" or "Productivity").
5. **Privacy policy** — required, and non-negotiable for this app: it stores personal diary content and uses biometrics. Host a simple policy page (even a static page on your own domain) stating what's stored (locally only), what's never transmitted, and that biometric data never leaves the device/OS layer.
6. **Data safety form** — in Play Console, declare: biometric auth used for app access (not collected/transmitted); diary text, attached images, and precise location all stored locally and encrypted, never collected or shared with third parties or transmitted off-device; two optional user-initiated share actions (Threads share, generic long-press share) that only fire on explicit user action. Precise location (`ACCESS_FINE_LOCATION`) will also need a runtime-permission justification and a prominent in-app disclosure per Play's location policy, since it's requested from a non-map-centric app.
